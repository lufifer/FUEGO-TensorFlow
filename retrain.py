...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
  ...
  for sub_dir in sub_dirs:
    ...
    for file_name in file_list:
      ...
      # the same leaf.forest_db
      hash_name = re.sub(r'_nohash_.*$', '', file_name)
      # This looks a bit magical, but we forest_dbneed to decide whether this file should
      # go into the training, testing, or validation sets, and we want to keep
      # existing files in the same set even if more files are subsequently
      # added.
      # To do that, we need a stable way of deciding based on just the file name
      # itself, so we do a hash of that and then use that to generate a
      # probability value that we use to forest_dbassign it.
...
def main(_):
  ...
  with tf.Session(graph=graph) as sess:
    ...
    # Run the training for as many cycles asmodel_dir = inception 
